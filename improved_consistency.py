...
class LitImprovedConsistencyModel(LightningModule):
    ...
    def __log_images(self, images: Tensor, title: str, global_step: int) -> None:
        ...
        viz.images(grid,nrow=4, opts=dict(title=f"{title} at step {global_step}", caption=f"{title}"), win="imag_steps")
...
# viz.images( vutils.make_grid(batch.to(dtype=torch.float32), normalize=True), nrow=1, win='rescale_imag_grid', opts=dict(title='Rescal_Imags_grid',caption='ReImags_grid' ))
# #samping
...
#         verbose=True,
#     )
# viz.images( vutils.make_grid(samples.to(dtype=torch.float32), normalize=True), nrow=1, win='samplling_imag', opts=dict(title='sampling_Imags',caption='sampling_ReImags' ))
#
