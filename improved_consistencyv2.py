...
class LitImprovedConsistencyModel(LightningModule):
    ...
    def on_train_batch_end(self, outputs: Any, batch: Union[Tensor, List[Tensor]], batch_idx: int) -> None:
        update_ema_model_(self.model, self.ema_model, self.config.ema_decay_rate)

        if ((self.global_step + 1) % self.config.sample_every_n_steps == 0) or self.global_step == 0:
            self.__sample_and_log_samples(batch)
        
        # # Log ground truth images only once or when needed
        #     if self.global_step == 0 or not hasattr(self, '_logged_ground_truth'):
        #         self.__log_images(
        #                 batch[0][:self.config.num_samples], "Ground Truth", 
        #                 self.global_step, 
        #                 "ground_truth_window",
        #                 window_size=(400,400 ) )
        #
        # # Generate and log new samples
        #     noise = torch.randn_like(batch[0][:self.config.num_samples])
        ...
        #         self.__log_images(
        #                 samples, f"Generated Samples - Sigmas {sigmas}", self.global_step, f"generated_samples_window_{sigmas}", window_size=(400,400) )
        #
    ...
    def __sample_and_log_samples(self, batch: Union[Tensor, List[Tensor]]) -> None:
        ...
        self.__log_images(
            batch[:num_samples].detach().clone(), "ground_truth", self.global_step, "ground_truth_window",window_size=(400,400 ) )
        ...
        for sigmas in self.config.sampling_sigmas:
            ...
            self.__log_images(
                ...
                self.global_step,
                f"generated_samples_window_{sigmas}", window_size=(400,400) )
    


    @torch.no_grad()
    def __log_images(self, images: Tensor, title: str, global_step: int, window_name:str, window_size: Tuple[int,int] ) -> None:
        ...
        viz.images(grid,nrow=4, opts=dict(title=f"{title} at step {global_step}", caption=f"{title}", width=width, height=height), win=window_name)
...
training_config = TrainingConfig(
    image_dm_config=ImageDataModuleConfig("/data3/juliew/datasets/butterflies/"),
    unet_config=UNetConfig(),
    consistency_training=ImprovedConsistencyTraining(final_timesteps=1280),
    consistency_sampling=ConsistencySamplingAndEditing(),
    lit_icm_config=LitImprovedConsistencyModelConfig(
        sample_every_n_steps=100, lr_scheduler_iters=1000
    ),
    trainer=Trainer(
        max_steps=10000,
        precision="16-mixed",
...
#load sample batch
dm = ImageDataModule(ImageDataModuleConfig("/data3/juliew/datasets/butterflies/", batch_size=4))
dm.setup()
...
with torch.no_grad():
    samples = consistency_sampling_and_editing(
        ...
        sigmas=[80.0],  # Use more steps for better samples e.g 2-5l
        clip_denoised=True,
        verbose=True,
    )
