...
import torchvision.utils as vutils
viz=Visdom(env='consistence_test_mask_emmu')
...
class ImprovedConsistencyTraining:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        mask_fill_one = T.RandomErasing(p=1.0, scale=(0.2, 0.4), ratio=(1.3, 3.3), value=1, inplace=True)
        ...
        viz.images( vutils.make_grid(next_x.cpu(), normalize=True, nrow=int(x.shape[0]/2)), win="consistence_test_emmu", opts=dict(title="train_next_x image", caption="train_next_x image", width=300, height=300) )
        ...
        with torch.no_grad():
            ...
            viz.images( vutils.make_grid(current_x.cpu(), normalize=True, nrow=int(x.shape[0]/2)), win="consistence_test_emmu2", opts=dict(title="train_current_x image", caption="train_current_x image", width=300, height=300) )
...
class ConsistencySamplingAndEditing:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        viz.images( vutils.make_grid(x.cpu(), normalize=True, nrow=int(x.shape[0]/2)), win="consistence_test_emmu3", opts=dict(title="sampling_x_T image", caption="sampling_x_T image", width=300, height=300) )

        # Progressively denoise the sample and skip the first step as it has already
        # been run
        pbar = tqdm(sigmas[1:], disable=(not verbose))
        mask_fill_one = T.RandomErasing(p=1.0, scale=(0.2, 0.2), ratio=(0.5, 0.5), value= 1 )     ##apply mask
        zero_tensor=torch.zeros_like(x)
        mask = mask_fill_one(zero_tensor)
        ...
        for sigma in pbar:
            ...
            viz.images( vutils.make_grid(x.cpu(), normalize=True, nrow=int(x.shape[0]/2)), win="consistence_test_emmu4", opts=dict(title="estimated_x_t image", caption="estimated_x_t image", width=300, height=300) )
            ...
            viz.images( vutils.make_grid(x.cpu(), normalize=True, nrow=int(x.shape[0]/2)), win="consistence_test_emmu5", opts=dict(title="function x_t image", caption="function x_t image", width=300, height=300) )
            if clip_denoised:
                x = x.clamp(min=-1.0, max=1.0)
            x = self.__mask_transform(x, y, mask, transform_fn, inverse_transform_fn)

            viz.images( vutils.make_grid(x.cpu(), normalize=True, nrow=int(x.shape[0]/2)), win="consistence_test_emmu6", opts=dict(title="final x_t image", caption="final x_t image", width=300, height=300) )
        return x
