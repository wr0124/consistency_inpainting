...
dm = ImageDataModule(ImageDataModuleConfig("/data3/juliew/datasets/butterflies/", batch_size=2))
dm.setup()
batch, _ = next(iter(dm.train_dataloader()))


batch = batch.to(device=device, dtype=dtype)
viz.images( vutils.make_grid(batch.to(dtype=torch.float32), normalize=True), nrow=1, win='batch_image', opts=dict(title='batch_image',caption='batch_image',width=300,height=300 ))
...
viz.images( vutils.make_grid(batch_a_b.to(dtype=torch.float32), normalize=True), nrow=1, win='batch_aclone_bflip3', opts=dict(title='batch_aclone_bflipAB',caption='batch_aclone_bflip',width=300,height=300 ))
...
viz.images( vutils.make_grid(interpolation.to(dtype=torch.float32), normalize=True), nrow=1, win='interpolation_a_b', opts=dict(title='interpolation_a_b',caption='interpolation_a_b',width=300,height=300 ))
...
for i in range(batch_size):
    ...
    sequence = torch.cat([batch_a_image, interpolated_for_batch_item, batch_b_image], dim=0)  # Shape: [7, 3, 32, 32]
...
viz.images( vutils.make_grid(interpolation_multip.to(dtype=torch.float32), normalize=True, nrow=7), 
                             win='interpolation_multip_a_b', opts=dict(title='interpolation_multip_a_b', caption='interpolation_multip_a_b',width=300,height=300 )) 
