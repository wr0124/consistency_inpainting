...
class LitImprovedConsistencyModel(LightningModule):
    ...
    def __sample_and_log_samples(self, batch: Union[Tensor, List[Tensor]]) -> None:
        ...
        for sigmas in self.config.sampling_sigmas:
            random_erasing = T.RandomErasing(p=1.0, scale=(0.2, 0.5), ratio=(0.5, 0.5), value='random')
            masked_batch = random_erasing(batch)
            mask = torch.logical_not(batch == masked_batch)


            samples = self.consistency_sampling(
                self.ema_model, batch, sigmas, clip_denoised=True, verbose=True
            )            # Generated samples
    ...
    def __log_images(self, images: Tensor, title: str, global_step: int, window_name:str, window_size: Tuple[int,int] ) -> None:
        ...
        viz.images(grid,nrow=6, opts=dict(title=f"{title} at step {global_step}", caption=f"{title}", width=width, height=height), win=window_name)
...
training_config = TrainingConfig(
    ...
    consistency_sampling=ConsistencySamplingAndEditing(),
    lit_icm_config=LitImprovedConsistencyModelConfig(sample_every_n_steps=1000, lr_scheduler_iters=1000),
    trainer=Trainer(
        max_steps=50000,
        precision="16-mixed",
